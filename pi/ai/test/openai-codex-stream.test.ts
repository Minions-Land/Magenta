...
describe("openai-codex streaming", () => {
	...
	it("clamps Codex session-id header to 64 characters", async () => {
		...
		// Magenta clamps overlong keys to a 64-char prefix-hash form rather than plain truncation.
		expect(capturedHeaders?.get("session-id")).toMatch(/^x{31}-[a-f0-9]{32}$/);
		expect(capturedHeaders?.get("x-client-request-id")).toMatch(/^x{31}-[a-f0-9]{32}$/);
