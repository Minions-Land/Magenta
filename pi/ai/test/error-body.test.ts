...
describe("normalizeProviderError", () => {
	...
	it("redacts authorization, API key, and token fields from structured bodies", () => {
		const error = Object.assign(new Error("403 status code (no body)"), {
			status: 403,
			error: {
				message: "gateway rejected credentials",
				authorization: "Bearer live-auth-token",
				api_key: "sk-live-api-key-123456",
				access_token: "eyJheader.payload.signature",
			},
		});

		const formatted = formatProviderError(normalizeProviderError(error));

		expect(formatted).toContain("gateway rejected credentials");
		expect(formatted).toContain("[REDACTED]");
		expect(formatted).not.toContain("live-auth-token");
		expect(formatted).not.toContain("sk-live-api-key-123456");
		expect(formatted).not.toContain("eyJheader.payload.signature");
	});

	it("redacts credentials from raw bodies and non-Error SDK values", () => {
		const rawBody = "Authorization: Bearer abc.def.ghi x-api-key=sk-secret12345678";
		const formatted = formatProviderError(
			normalizeProviderError(Object.assign(new Error("gateway failure"), { statusCode: 502, body: rawBody })),
		);
		const serialized = formatProviderError(
			normalizeProviderError({ message: "failed", headers: { Authorization: "Bearer raw-secret-token" } }),
		);

		expect(formatted).not.toContain("abc.def.ghi");
		expect(formatted).not.toContain("sk-secret12345678");
