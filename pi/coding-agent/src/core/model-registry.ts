...
export { applyModelOverride, clearApiKeyCache } from "./provider-composer.ts";
...
export class ModelRegistry {
	...
	/** @internal Access the underlying runtime (single owner). Used by AgentSession wiring. */
	...
	getProviderDisplayName(provider: string): string {
		// Explicit extension-registered names win, then the branded built-in display
		// map, then the pi-ai provider's own name, then the raw provider id.
		const registered = this.runtime.getRegisteredProviderConfig(provider);
		return (
			registered?.name ??
			registered?.oauth?.name ??
			BUILT_IN_PROVIDER_DISPLAY_NAMES[provider] ??
			this.runtime.getProvider(provider)?.name ??
			provider
		);
	}
