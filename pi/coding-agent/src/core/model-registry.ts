...
import { loadExternalAuth } from "./external-auth-loader.ts";
...
export class ModelRegistry {
	...
	private loadModels(): void {
		...
		// Apply baseUrl overrides from external auth sources (Claude Code, Codex, env tokens).
		// This lets a custom ANTHROPIC_BASE_URL / OPENAI_BASE_URL automatically reroute
		// built-in models without requiring a models.json entry. Explicit models.json
		// overrides take precedence (we only fill in providers not already overridden).
		for (const cred of loadExternalAuth()) {
			if (cred.baseUrl && !overrides.has(cred.provider)) {
				overrides.set(cred.provider, { baseUrl: cred.baseUrl });
			}
		}
