...
export class AuthStorage {
	...
	hasAuth(provider: string): boolean {
		...
		// Check external auth sources (Claude Code, Codex, env tokens)
		if (getExternalAuth(provider)?.apiKey) return true;
		return false;
	}
	...
	getAuthStatus(provider: string): AuthStatus {
		...
		// Check external auth sources (Claude Code, Codex, env tokens)
		const externalCred = getExternalAuth(provider);
		if (externalCred?.apiKey) {
			return { configured: true, source: "environment", label: `${externalCred.source}` };
		}
	...
	async getApiKey(providerId: string, options: GetApiKeyOptions = {}): Promise<string | undefined> {
		...
		// Try external auth sources (Claude Code, Codex, env tokens) before fallback.
		// These are treated as explicitly-configured credentials (not env fallback),
		// so they apply even when includeFallback is false.
		const externalCred = getExternalAuth(providerId);
		if (externalCred?.apiKey) {
			return externalCred.apiKey;
		}
