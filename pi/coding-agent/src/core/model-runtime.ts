...
export interface CreateModelRuntimeOptions {
	/** Credential storage. Defaults to a Magenta composite store (AuthStorage + external files). */
...
/** Case-insensitive header merge: override wins, removing any prior casing variant. */
...
export class ModelRuntime implements Models {
	...
	static async create(options: CreateModelRuntimeOptions = {}): Promise<ModelRuntime> {
		const baseStore = options.credentials ?? createMagentaCredentialStore(AuthStorage.create(options.authPath));
	...
	/**
	 * Final request auth/header assembly (CC-050). Order:
	 *   provider/config/model auth headers -> attribution (applied upstream in model) ->
	 *   [before_provider_headers ext transform slot, CC-027 Phase 4] ->
	 *   cache/wire observation -> dispatch.
	 * The transformHeaders slot is invoked here after auth headers are merged so a
	 * later Phase 4 hook can rewrite the final header set case-insensitively.
	 */
	private async prepareRequest(
		...
		// before_provider_headers extension transform slot (CC-027, Phase 4).
