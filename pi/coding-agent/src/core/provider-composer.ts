...
export function applyModelOverride(model: Model<Api>, override: ModelsJsonModelOverride): Model<Api> {
	...
	// Merge cost: tiered replaces entirely, partial flat merges per-field (AI-023/CC-037).
	// W5 ModelCost is a union: flat {input,output,cacheRead,cacheWrite} or {tiers:{default,scale}}.
	if (override.cost) {
		if ("tiers" in override.cost) {
			result.cost = override.cost;
		} else if ("tiers" in model.cost) {
			// Base tiered, override flat: apply flat as a partial update to the default tier.
...
function modelFromJson(
	...
	return {
		...
		thinkingLevelMap:
			definition.thinkingLevelMap ?? inferThinkingLevelMap(definition.id, api as Api, definition.reasoning ?? false),
