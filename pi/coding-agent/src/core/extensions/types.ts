...
/**
 * Fired after request headers are assembled (auth + attribution), before the
 * provider HTTP call. Handlers mutate `headers` in place (e.g. to inject
 * tracing/session headers); the return value is ignored. A `null` value deletes
 * that header. Header names are matched case-insensitively when merged.
 */
...
export type InputSource = "interactive" | "print" | "json" | "rpc" | "extension";
...
export interface ProviderModelConfig {
	...
	input: ("text" | "image")[];
	/** Cost per token (for tracking, can be flat or volume-tiered). */
	cost: Model<Api>["cost"];
