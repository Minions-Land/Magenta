/**
 * Per-provider pi.dev catalog overlay (CC-052 / CC-058, upstream cd7cad4e +
 * fab309e9/bd9e09db/97f9978f).
 *
 * `withRemoteCatalog` wraps a static built-in provider so its model list can be
 * augmented by a dynamically refreshed catalog fetched from pi.dev and persisted
 * through the provider-scoped ModelsStore. Refreshes are throttled to once every
 * four hours, unimplemented routes (404/501) collapse to an empty overlay, and
 * the request carries the Magenta branded user agent (VERSION = BRAND_VERSION)
 * without ever sending the provider credential to the catalog host.
 */
