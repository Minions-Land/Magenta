...
export class EditorComponentWrapper implements EditorComponent {
	...
	protected inner: EditorComponent;

	constructor(inner: EditorComponent) {
		this.inner = inner;
	}

	get onEscape(): (() => void) | undefined {
		return (this.inner as EditorComponent & { onEscape?: () => void }).onEscape;
	}
	set onEscape(handler: (() => void) | undefined) {
		(this.inner as EditorComponent & { onEscape?: () => void }).onEscape = handler;
	}
	get onCtrlD(): (() => void) | undefined {
		return (this.inner as EditorComponent & { onCtrlD?: () => void }).onCtrlD;
	}
	set onCtrlD(handler: (() => void) | undefined) {
		(this.inner as EditorComponent & { onCtrlD?: () => void }).onCtrlD = handler;
	}
	get onPasteImage(): (() => void) | undefined {
		return (this.inner as EditorComponent & { onPasteImage?: () => void }).onPasteImage;
	}
	set onPasteImage(handler: (() => void) | undefined) {
		(this.inner as EditorComponent & { onPasteImage?: () => void }).onPasteImage = handler;
	}
	get onExtensionShortcut(): ((data: string) => boolean) | undefined {
		return (this.inner as EditorComponent & { onExtensionShortcut?: (data: string) => boolean }).onExtensionShortcut;
	}
	set onExtensionShortcut(handler: ((data: string) => boolean) | undefined) {
		(this.inner as EditorComponent & { onExtensionShortcut?: (data: string) => boolean }).onExtensionShortcut =
			handler;
	}
	...
	onAction(action: AppKeybinding, handler: () => void): void {
		...
		const innerWithAction = this.inner as EditorComponent & {
			onAction?: (action: AppKeybinding, handler: () => void) => void;
		};
		innerWithAction.onAction?.(action, handler);
	}
	invalidate(): void {
	...
	isShowingAutocomplete(): boolean {
		return (
			(this.inner as EditorComponent & { isShowingAutocomplete?: () => boolean }).isShowingAutocomplete?.() ?? false
		);
	}
}
