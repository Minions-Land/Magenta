...
export function installSubAgents(pi: ExtensionAPI, eventsMonitor: EventsMonitor) {
	let nextAgentNumber = 1;
	let shuttingDown = false;
	const events = new Map<string, SubAgentEvent>();
	...
	async function startSubAgent(input: AgentTask, parentCwd: string): Promise<SubAgentEvent> {
	...
	function resolveEventIds(eventId?: string, eventIds?: string[]): string[] {
		const ids = [...(eventIds ?? [])];
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, signal, _onUpdate, ctx) {
			...
			if (action === "status") {
				...
				if (!ids.length) return { content: [{ type: "text", text: "No sub-agents." }], details: { events: 0 } };
