...
import {
	CLIPBOARD_PATH_RE,
	IMAGE_FILE_RE,
	MACOS_CLIPBOARD_FILE_PATHS_SCRIPT,
	TOKEN_LINE_RE,
	TOKEN_RE,
} from "./constants.ts";
...
class ImageTokenController {
	...
	constructor(attachments: Map<string, Attachment>) {
	...
	findDeleteRange(
		...
		cursorCol: number,
		backward: boolean,
	): { start: number; end: number; token: string } | undefined {
...
class ImageEditor extends Editor {
	...
	scheduleScan(): void {
		...
		this.scanTimers = [80, 250, 600].map((delay) =>
			setTimeout(() => this.imageTokens.replaceClipboardPathsInEditor(this, this.tui), delay),
		);
	}
}
...
class ImageEditorWrapper extends EditorComponentWrapper {
	...
	scheduleScan(): void {
		for (const timer of this.scanTimers) clearTimeout(timer);
		this.scanTimers = [80, 250, 600].map((delay) =>
			setTimeout(() => this.imageTokens.replaceClipboardPathsInEditor(this.inner, this.tui), delay),
		);
	}
}
	...
	return {
		...
		createEditorFactory(ctx: {
			...
			return (tui: TUI, theme: EditorTheme, keybindings: KeybindingsManager): EditorComponent => {
				if (!previousEditorFactory)
					return new ImageEditor(tui, theme, keybindings, imageTokens, () => ctx.ui.theme);
				return new ImageEditorWrapper(
		...
		transformInput(text: string): { action: "continue" } | { action: "transform"; text: string } {
			...
			const transformed = text.replace(
				TOKEN_RE,
				(full, id) => attachments.get(imageToken(Number(id)))?.path ?? full,
			);
