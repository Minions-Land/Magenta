...
function renderCodeBlock(md: MarkdownRuntime, token: MarkdownToken, width: number, next?: string): string[] {
	...
	if (PLAIN_CODE_LANGS.has(lang.toLowerCase()) || width < 12) {
		...
		return withGap(
			code.split("\n").map((line) => indent + md.theme.codeBlock(line)),
			next,
		);
	}
	...
	const highlighted = md.theme.highlightCode
		? md.theme.highlightCode(code, lang)
...
function renderList(
	...
	for (let i = 0; i < (token.items ?? []).length; i++) {
		...
		for (const child of item.tokens ?? []) {
			...
			const childLines =
				renderMarkdownToken(md, child, itemWidth, undefined, styleContext) ??
				original?.call(md, child, itemWidth, undefined, styleContext) ??
				[];
...
function renderBlockquote(
	...
	for (const child of token.tokens ?? []) {
		rendered.push(
			...(renderMarkdownToken(md, child, quoteWidth, undefined, styleContext) ??
				original?.call(md, child, quoteWidth, undefined, styleContext) ??
				[]),
		);
	}
	...
	const lines = (rendered.length ? rendered : [""]).flatMap((line) =>
		wrapTextWithAnsi(line, quoteWidth).map(
			(wrapped) => md.theme.quoteBorder("▌ ") + md.theme.quote(md.theme.italic(wrapped)),
		),
	);
	return withGap(lines, next);
}
...
function renderMarkdownToken(
	...
	if (isToken(token, "table") && original) {
		return original
			.call(md, token, width, next, styleContext)
			.map((line) => line.replace(/[┌┬┐├┼┤└┴┘│─]/g, (char) => md.theme.codeBlockBorder(char)));
	}
...
export function installMarkdownPatch(): void {
	...
	proto.renderToken = function (
		...
		return (
			renderMarkdownToken(this, token, width, next, styleContext) ??
			original.call(this, token, width, next, styleContext)
		);
	};
}
