...
import { ENV_AGENT_DIR, ENV_PEER_MESSAGE_DB, isBunBinary, VERSION } from "../config.ts";
...
export async function installUpdate(): Promise<UpdateInstallResult> {
	...
	try {
		...
		if (platform() === "win32") {
			await prepareWindowsReleaseUpdateTransaction({
				installDirectory,
				operationId,
				currentBinary,
				stagingDirectory,
		...
		if (await shouldSkipConcurrentUpdateTransaction(installDirectory, installedVersion, checkResult.latestVersion)) {
