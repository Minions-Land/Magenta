...
import { MessageStore, type PeerEndpointObservedState, shellQuote } from "@magenta/harness";
import { getPeerMessageDbPath } from "../config.ts";
import { peerEndpointId } from "../core/peer-endpoint.ts";
import { PeerLinkSession } from "../core/peer-link-session.ts";
import { MessageStorePeerLinkAdapter } from "../core/peer-link-store-adapter.ts";
...
async function runPeerRelay(args: string[]): Promise<void> {
	const dbPath = option(args, "--db") ?? getPeerMessageDbPath();
...
async function runDemoCommand(command: string, args: string[]): Promise<void> {
	const dbPath = option(args, "--db") ?? getPeerMessageDbPath();
	const store = new MessageStore(dbPath);
	try {
		switch (command) {
...
export async function handlePeerCommand(args: string[]): Promise<boolean> {
	if (args[0] !== "_peer") return false;
	const command = args[1];
	if (!command) throw new Error("_peer requires a command");
	if (command === "link") {
		await runPeerLinkResponder(option(args, "--db") ?? getPeerMessageDbPath());
		return true;
	}
	if (command === "relay") {
		await runPeerRelay(args.slice(2));
		return true;
	}
	await runDemoCommand(command, args.slice(2));
	return true;
}
