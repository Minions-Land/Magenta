...
Magenta uses JSON settings files with project settings overriding global settings.

| Location | Scope |
|----------|-------|
| `~/.magenta/agent/settings.json` | Global (all projects) |
| `.magenta/settings.json` | Project (current directory) |
...
`enableInstallTelemetry` only controls the anonymous install/update ping to `https://pi.dev/api/report-install`. Opting out of telemetry does not disable update checks; Magenta can still fetch `https://pi.dev/api/latest-version` to look for the latest version.
...
Local cache telemetry does not persist prompt, system, tool-definition, tool-output, or provider-output text. It stores HMAC fingerprints, structural byte/item counts, normalized breakpoint metadata, timing, usage, and cost. The local HMAC key and per-recorder JSONL event logs are created with owner-only permissions; concurrent Magenta processes use independent active files so their rotation cannot replace another process's log. Records identify whether a payload was observed at the actual Codex wire boundary, at a provider payload boundary, or only as a custom-provider logical fallback; fallback and structurally unclassified provider payloads are not treated as evidence that cache reuse was expected.
...
Keep `retry.provider.maxRetries` at `0` unless provider-level retries are explicitly needed. Setting it above `0` can make SDK/provider retries handle out-of-usage-limit errors before Magenta sees them, which may block the agent until the provider quota resets in some circumstances.
...
`npmCommand` is used for all npm package-manager operations, including installs, uninstalls, and dependency installs inside git extension packages. User-scoped npm packages install under `~/.magenta/agent/npm/`; project-scoped npm packages install under `.magenta/npm/`. Use argv-style entries exactly as the process should be launched. When `npmCommand` is configured, git package dependency installs use plain `install` to avoid npm-specific flags in wrappers or alternate package managers.
...
```json
{ "sessionDir": ".magenta/sessions" }
```

When multiple sources specify a session directory, precedence is `--session-dir`, `MAGENTA_CODING_AGENT_SESSION_DIR`, then `sessionDir` in settings.json.
...
Paths in `~/.magenta/agent/settings.json` resolve relative to `~/.magenta/agent`. Paths in `.magenta/settings.json` resolve relative to `.magenta`. Absolute paths and `~` are supported.
...
Project settings (`.magenta/settings.json`) override global settings. Nested objects are merged:

```json
// ~/.magenta/agent/settings.json (global)
{
  "theme": "dark",
  "compaction": { "enabled": true, "reserveTokens": 16384 }
}

// .magenta/settings.json (project)
{
