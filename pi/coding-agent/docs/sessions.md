...
Magenta saves conversations as sessions so you can continue work, branch from earlier turns, and revisit previous paths.

## Session Storage

Sessions auto-save to `~/.magenta/agent/sessions/`, organized by working directory. Each session is a JSONL file with a tree structure.

```bash
magenta -c                  # Continue most recent session
magenta -r                  # Browse and select from past sessions
magenta --no-session        # Ephemeral mode; do not save
magenta --name "my task"    # Set session display name at startup
magenta --session <path|id> # Use a specific session file or partial session ID
magenta --fork <path|id>    # Fork a session file or partial session ID into a new session
```
...
`/resume` opens an interactive session picker for the current project. `magenta -r` opens the same picker at startup.
...
When available, Magenta uses the `trash` CLI for deletion instead of permanently removing files.
...
```bash
magenta --name "Refactor auth module"
magenta --name "CI audit" -p "Review this build failure"
```

Named sessions are easier to find in `/resume` and `magenta -r`.
...
When `/tree` switches away from one branch to another, Magenta can summarize the abandoned branch and attach that summary at the new position. This preserves important context from the path you left without replaying the whole branch.
