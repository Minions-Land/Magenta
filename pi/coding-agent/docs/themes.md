> Magenta can create themes. Ask it to build one for your setup.
...
Magenta loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.magenta/agent/themes/*.json`
- Project: `.magenta/themes/*.json` (only after the project is trusted)
...
On first run, Magenta detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.magenta/agent/themes
vim ~/.magenta/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/Minions-Land/Magenta/main/pi/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, Magenta reloads it automatically for immediate visual feedback.
...
Magenta uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, Magenta falls back to the nearest approximation.
