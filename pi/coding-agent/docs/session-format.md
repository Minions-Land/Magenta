...
```
~/.magenta/agent/sessions/--<path>--/<timestamp>_<uuid>.jsonl
```
...
Sessions can be removed by deleting their `.jsonl` files under `~/.magenta/agent/sessions/`.

Magenta also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, Magenta uses the `trash` CLI to avoid permanent deletion.
...
Source files:
- [`src/core/session-manager.ts`](../src/core/session-manager.ts) - Session entry types and SessionManager
- [`src/core/messages.ts`](../src/core/messages.ts) - Extended message types (BashExecutionMessage, CustomMessage, etc.)
- [`pi/ai/src/types.ts`](../../ai/src/types.ts) - Base message types (UserMessage, AssistantMessage, ToolResultMessage)
- [`pi/agent/src/types.ts`](../../agent/src/types.ts) - AgentMessage union type
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if Magenta-generated (legacy field name)
...
`buildContextEntries()` walks from the current leaf to the root, producing the entry list for rendering and context:

1. Collects all entries on the path
2. If a `CompactionEntry` is on the path:
   - Emits the compaction entry first
