...
The extension factory can also be `async`. For dynamic model discovery, fetch and register models in the factory instead of `session_start`. Magenta waits for the factory before startup continues, so the provider is available during interactive startup and to `magenta --list-models`.
...
Calls made after the initial extension load phase are applied immediately, so no `/refresh` is required.
...
models: [{
  ...
  // ...
  reasoning: true,
  thinkingLevelMap: {              // map library levels to provider values; null hides unsupported levels
    minimal: null,
    low: null,
    medium: null,
    high: "default",
...
Credentials are persisted in `~/.magenta/agent/auth.json`:
...
- [anthropic.ts](../../ai/src/providers/anthropic.ts) - Anthropic provider
- [mistral.ts](../../ai/src/providers/mistral.ts) - Mistral provider
- [openai-completions.ts](../../ai/src/api/openai-completions.ts) - OpenAI Chat Completions API
- [openai-responses.ts](../../ai/src/api/openai-responses.ts) - OpenAI Responses API
- [google.ts](../../ai/src/providers/google.ts) - Google provider
- [amazon-bedrock.ts](../../ai/src/providers/amazon-bedrock.ts) - AWS Bedrock provider
...
When a request exceeds the model's context window, Magenta can recover automatically by compacting the conversation and retrying. This recovery only kicks in if Magenta recognizes the failure as an overflow.

Detection runs on the finalized assistant message:

- `stopReason === "error"`
- `errorMessage` matches one of Magenta's known overflow patterns (see [`pi/ai/src/utils/overflow.ts`](../../ai/src/utils/overflow.ts))

If your provider returns overflow errors with a message Magenta does not recognize, normalize the error from the same extension that registers the provider. Use a `message_end` handler to rewrite the assistant message so its `errorMessage` starts with a phrase Magenta recognizes. The generic fallback `context_length_exceeded` is the safest choice.
...
`message_end` runs before Magenta tracks the assistant message for auto-compaction, so the rewritten `errorMessage` is what Magenta checks. With this in place, Magenta will:
...
- Match a provider-specific pattern, not Magenta's generic overflow patterns. Rewriting rate-limit or throttling errors (`rate limit`, `too many requests`) would falsely trigger compaction instead of Magenta's normal retry-with-backoff path.
...
Test your provider against the same test suites used by built-in providers. Copy and adapt these test files from [`pi/ai/test/`](../../ai/test/):
...
interface ProviderModelConfig {
  ...
  /** Maps library thinking levels to provider/model-specific values; null marks a level unsupported. */
