...
- Unix entrypoint transactions now bind legacy files to nanosecond birth/modify times, size, mode, device, and inode, so Linux inode reuse cannot hide a replacement made between preflight and atomic activation or recovery
...
- Public Unix installation now scopes the release-bound bootstrap to `v0.1.0+`; the bootstrap gives `v0.0.29` users an explicit fail-closed handoff to a fixed-tag, fixed-manifest manual transition instead of attempting an unbound installer download
...
- The downstream public verifier now pins its Apple Team ID in reviewed repository source, anonymously verifies the public source's exact annotated tag without a cross-repository secret, downloads Windows assets by immutable API ID with streaming limits, and runs both native macOS architectures to bind all six materialized helper bytes and signatures to the signing receipt; source publication refuses to proceed unless the two repositories' trust files match exactly
- Ordinary source branches and tags no longer retain the revoked historical GitHub token: an exact replacement preserved the current tree while rewriting only the four audited paths, and release maintenance now records that GitHub's hidden pull-request ref still requires a server-side sensitive-data purge
