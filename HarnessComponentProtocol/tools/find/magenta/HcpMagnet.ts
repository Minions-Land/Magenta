...
export class HcpMagnet {
	...
	static async build(context: HcpMagnetBuildContext) {
		if (!context.descriptorPath) throw new Error("tools/find:magenta requires a component descriptor");
		...
		if (!runtime || !sandbox) throw new Error("tools/find:magenta requires runtime:process and sandbox");
