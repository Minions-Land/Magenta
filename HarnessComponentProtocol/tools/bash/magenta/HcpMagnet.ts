...
export class HcpMagnet {
	...
	static async build(context: HcpMagnetBuildContext) {
		if (!context.descriptorPath) throw new Error("tools/bash:magenta requires a component descriptor");
		...
		if (!runtime || !sandbox) throw new Error("tools/bash:magenta requires runtime:process and sandbox");
