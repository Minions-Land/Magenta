...
export type JsonRpcPeerOptions = {
	...
	/**
	 * Optional transport-specific error sanitizer. HTTP clients use this to keep
	 * credentials echoed by a remote server out of diagnostics; stdio callers do
	 * not provide it and retain the historical error text.
	 */
	sanitizeErrorMessage?: (message: string) => string;
};

export type JsonRpcRequestOptions = {
	/** Called when correlation times out so a transport can abort its I/O. */
	onTimeout?: () => void;
};
...
export class JsonRpcPeer {
	...
	private readonly sanitizeErrorMessage?: (message: string) => string;
	...
	/** Whether one specific request id is still awaiting a response. */
	 ...
	 */
	createRequest(method: string, params: unknown, options: JsonRpcRequestOptions = {}): JsonRpcOutgoing {
	...
	handleMessage(raw: string): boolean {
		...
		return this.handleParsed(message);
	}

	/** Match an already-parsed JSON-RPC message to a pending request. */
	handleParsed(message: JsonRpcResponse): boolean {
		if (!message || typeof message !== "object") return false;
		if (message.jsonrpc !== JSONRPC_VERSION || typeof message.id !== "number" || message.method !== undefined) {
			return false;
		}
		const hasResult = Object.hasOwn(message, "result");
		const hasError = Object.hasOwn(message, "error");
		if (hasResult === hasError) return false;
		if (hasError && (!message.error || typeof message.error !== "object")) return false;
		...
		if (message.error) {
			...
			const errorMessage = `MCP error ${message.error.code ?? ""}: ${detail}`.trim();
			pending.reject(new Error(this.sanitizeErrorMessage?.(errorMessage) ?? errorMessage));
			return true;
		}
