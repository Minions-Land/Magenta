...
import * as contextServer from "../context/HcpServer.ts";
...
describe("context provider", () => {
	...
	it("serves context through context://workspace and context://project", async () => {
		...
		const source = {
			kind: "native",
			hotSwappable: false,
			toCapability: () => ({ kind: "context", name: "context", source: "magenta", instance: provider }),
		};
		const hcp = new HcpClient();
		hcp.registerModule(new contextServer.HcpServer(), new Map([["context", source]]));
