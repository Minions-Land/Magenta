...
import type {
	ProcessExecInput,
	ProcessExecOutput,
	ProcessRuntimeManagedExit,
	ProcessRuntimeManagedHandle,
	ProcessRuntimeManagedInput,
...
const MANAGED_PROCESS_KILL_GRACE_MS = 1_000;
...
function sandboxProfile(input: ProcessExecInput): Record<string, unknown> | undefined {
	...
	const wrapped = "profile" in sandbox ? sandbox.profile : undefined;
...
function spawnWithPolicy(
	input: Pick<ProcessExecInput, "command" | "args" | "env_overrides">,
...
type ManagedProcessTermination = Pick<ProcessRuntimeManagedExit, "reason" | "error">;
...
class ManagedProcessHandle {
	...
	private readonly stdoutBacklog: string[] = [];
	private readonly stderrBacklog: string[] = [];
	private stdoutRemainder = "";
	private finished = false;
	private termination?: ManagedProcessTermination;
	private timeout?: ReturnType<typeof setTimeout>;
	private forceKillTimeout?: ReturnType<typeof setTimeout>;
	private readonly onAbort: () => void;
	private resolveExit!: (result: ProcessRuntimeManagedExit) => void;

	constructor(child: ChildProcessWithoutNullStreams, signal: AbortSignal | undefined, timeoutMs: number | undefined) {
		this.child = child;
		this.signal = signal;
		...
		this.onAbort = () => {
			this.terminate({ reason: "abort", error: new Error("Operation aborted") });
		};

		child.stdout.setEncoding("utf-8");
		child.stderr.setEncoding("utf-8");
		child.stdout.on("data", (chunk: string) => this.consumeStdout(chunk));
		child.stdout.once("end", () => this.flushStdoutRemainder());
		child.stderr.on("data", (chunk: string) => this.emitStderr(chunk));
		...
		child.once("close", (status, exitSignal) => {
			...
			this.finish({
				status,
				signal: exitSignal,
				reason: termination?.reason ?? "exit",
		...
		if (timeoutMs !== undefined && timeoutMs > 0) {
			this.timeout = setTimeout(() => {
				this.terminate({
					reason: "timeout",
					error: new Error(`process exceeded sandbox wall time of ${timeoutMs / 1000}s`),
				});
			}, timeoutMs);
		}
	}
	...
	write(data: string): Promise<void> {
		if (this.finished || this.child.stdin.destroyed || !this.child.stdin.writable) {
			return Promise.reject(new Error("managed process stdin is not writable"));
		}
	...
	onStdoutLine(listener: (line: string) => void): () => void {
		...
		for (const line of this.stdoutBacklog.splice(0)) listener(line);
		return () => this.stdoutListeners.delete(listener);
	}

	onStderr(listener: (chunk: string) => void): () => void {
		this.stderrListeners.add(listener);
		for (const chunk of this.stderrBacklog.splice(0)) listener(chunk);
		return () => this.stderrListeners.delete(listener);
	}

	async close(): Promise<void> {
		if (!this.finished) this.terminate({ reason: "close" });
		await this.exit;
	}
	...
	private emitStdoutLine(line: string): void {
		if (this.stdoutListeners.size === 0) {
			this.stdoutBacklog.push(line);
			return;
		}
	...
	private emitStderr(chunk: string): void {
		if (this.stderrListeners.size === 0) {
			this.stderrBacklog.push(chunk);
			return;
		}
		for (const listener of this.stderrListeners) listener(chunk);
	}

	private terminate(termination: ManagedProcessTermination): void {
		if (this.finished || this.termination) return;
		this.termination = termination;
		this.child.kill("SIGTERM");
		this.forceKillTimeout = setTimeout(() => {
			if (!this.finished) this.child.kill("SIGKILL");
		}, MANAGED_PROCESS_KILL_GRACE_MS);
		this.forceKillTimeout.unref();
	}

	private finish(result: ProcessRuntimeManagedExit): void {
...
async function spawnManagedProcess(
	...
	if (!input.command) throw new Error("runtime://process managed spawn requires command");
	if (signal?.aborted) throw new Error("Operation aborted");

	const policy = await policyFromInput(input);
	await validatePolicy(policy, input);
	if (signal?.aborted) throw new Error("Operation aborted");
	const child = spawnWithPolicy(input, policy);
	const timeoutMs = policy.maxWallSeconds > 0 ? policy.maxWallSeconds * 1000 : undefined;
	return new ManagedProcessHandle(child, signal, timeoutMs);
}
...
export class ProcessRuntimeProvider {
	...
	spawnManaged(input: ProcessRuntimeManagedInput, signal?: AbortSignal): Promise<ProcessRuntimeManagedHandle> {
		return spawnManagedProcess(input, signal);
	}
