import type { HookDescriptor, HookDiscoverResult, HookResult } from "../HcpServer.ts";
...
function isShellTool(input: unknown): boolean {
	...
	return name.toLowerCase() === "bash" || tags.includes("shell");
}
...
export function runLifecycleHook(name: string, input: unknown): HookResult {
	...
	if (name === "pre-tool") {
		const shellPolicy = isShellTool(input);
		actions.push({
			type: "hcp_call",
		...
		data = {
			...
			...(shellPolicy ? { shell_policy: { target: "shell://policy", op: "classify" } } : {}),
		};
	}
