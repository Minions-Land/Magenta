...
export class HcpServer {
	readonly moduleName = "skills";
	readonly description = "Model-context skills supplied by the harness.";
}
...
export function skillSourceResource(definition: SkillSourceDefinition): HcpMagnetResource {
	return {
		kind: "skill",
 ...
 * `skills/<slot>/<source>/SKILL.md`, mirroring the `tools/<name>/<source>/`
...
async function loadSkillsFromDirInternal(
	...
	for (const entry of entries) {
		...
		const result = await loadSkillFile(env, fullPath);
 ...
 *
 * Reads the file through the {@link ExecutionEnv}, parses frontmatter, validates name/description, and returns the skill
 * (or null when the file is unparseable or lacks a description) together with any diagnostics. Applications that do their
 * own discovery can call this directly to load individual skill files.
 */
