...
- `multiagent` is the durable lifecycle control plane for persistent teammate Sessions. Use it when retained context, repeated collaboration, or explicit worktree ownership matters. Target lifecycle actions only by Session id; use `send_message` for all ordinary prompts, progress, questions, blockers, and results. For editing, request `workspace="worktree"`, stop the Session after its explicit report, and then integrate or discard the verified receipt. Main shutdown preserves desired-running state and unintegrated generations for exact-lineage recovery.
- `send_message` atomically accepts one urgent message for a known Session mailbox. It does not create or manage a teammate, prove recipient consumption, or carry Assignment status; use `multiagent` for persistent lifecycle.
...
Run independent investigations in parallel when their inputs and file ownership do not conflict. Prefer read-only `sub_agent` workers for bounded analysis and synthesize their results in Main. Choose a persistent teammate for retained continuity or non-overlapping worktree ownership.

### Delegation ownership

Delegation uses **soft leases**, not runtime locks. Do not claim that the harness locks files, blocks `bash`, or prevents another actor from writing. Do not add an Assignment entity, Todo owner schema, or separate lease registry; running finite Events plus Main Todo work-unit boundaries and explicit Session messages are the coordination evidence.
...
- Main records persistent teammate scope in its Todo and communicates that scope in `send_message` text. For editing work, name owned files or globs and use `workspace="worktree"`. The linked checkout isolates ordinary Git paths, but it is not a security sandbox and does not intercept absolute-path writes.
- While a lease is active, do not redo the same task. Main may advance only non-overlapping Todo work, coordination, and integration preparation.
- A teammate becoming idle does not release its scope. Final assistant output is private; require the teammate to send an explicit result or blocker to Main, then reconcile that evidence into Todo. Do not poll status for completion.
...
- On finite failure, timeout, or cancellation, reclaim scope only after the terminal Event arrives. For a persistent teammate, reclaim or replace scope only after explicit Main coordination and hard interrupt/stop settlement where applicable.
- A teammate's explicit result message lets Main decide whether to release its Todo scope; there is no automatic terminal receipt. For worktree edits, stop the Session, and on a later turn take one status snapshot that shows non-live observed state before integrating the verified receipt, and independently verify the unstaged Main changes. Do not infer release from silence or idle status.
