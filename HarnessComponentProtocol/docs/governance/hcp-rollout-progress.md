...
Status: **HISTORICAL SNAPSHOT (2026-07-03).** This tracker records the earlier
rollout and intentionally preserves then-current names, paths, counts, and
commands. It is not implementation guidance. Current authority is
`hcp-architecture.md` and `hcp-naming.md`.

Original authoritative spec: `hcp-architecture.md` (this dir). The text below
is retained as the rollout disk-of-record rather than rewritten to resemble the
