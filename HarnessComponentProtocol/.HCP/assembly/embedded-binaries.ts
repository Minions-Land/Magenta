...
function getEmbeddedBinaryPath(): string | null {
	const binaryName = getEmbeddedBinaryName();
	
	try {
		// Bun 编译时，会根据 target 平台只打包对应的二进制
		// 这里的路径会在编译时被解析为虚拟路径
		if (PLATFORM === "darwin" && ARCH === "arm64") {
			return require("../../_magenta/process-tools/prebuilt/magenta-process-tools-macos-arm64");
		} else if (PLATFORM === "darwin" && ARCH === "x64") {
			return require("../../_magenta/process-tools/prebuilt/magenta-process-tools-macos-x64");
		} else if (PLATFORM === "linux" && ARCH === "x64") {
			return require("../../_magenta/process-tools/prebuilt/magenta-process-tools-linux-x64");
		} else if (PLATFORM === "win32" && ARCH === "x64") {
			return require("../../_magenta/process-tools/prebuilt/magenta-process-tools-windows-x64.exe");
		}
	} catch {
		// 开发环境或未嵌入场景
		return null;
...
export function getProcessToolsBinaryPath(): string {
	...
	if (!embeddedPath) {
		const devPath = join(
			process.cwd(),
			"HarnessComponentProtocol/_magenta/process-tools/prebuilt",
		...
		const releasePath = join(
			process.cwd(),
			"HarnessComponentProtocol/_magenta/process-tools/target/release",
			PLATFORM === "win32" ? "magenta-process-tools.exe" : "magenta-process-tools"
		);
		return releasePath;
	}
 ...
 * 2. 创建符号链接到 HCP_ROOT/../_magenta/process-tools/target/release/
 * 3. 这样 .toml 中的相对路径 "../../../_magenta/process-tools/target/release/magenta-process-tools" 可以正常工作
 */
export function initProcessToolsBinary(hcpRoot: string): void {
	try {
