...
|---|---|---|
| `pi.registerTool({ name, parameters, execute })` | **Tool** | `NativeToolMagnet` (native TS) or process Magnet (needs runtime). `toTool()` yields the loop-ready `AgentTool` |
| `pi.on("tool_call", ...)` gating/mutation | **Capability** (policy) | `CapabilitySourceMagnet` registered in `policy/sources.ts` |
...
- Factory function: `create<Name>Magnet`
...
[exports]
module = "pi/my-tool.ts"
factory = "createMyToolMagnet"
...
**Native tool Magnet:**
```typescript
import { NativeToolMagnet } from "@magenta/harness/hcp-magnet";

export function createMyToolMagnet(cwd: string): NativeToolMagnet {
  return new NativeToolMagnet({
    name: "my-tool",
    description: "...",
    parameters: {...},
    execute: async (params) => {
      return await executeMyTool(params, { cwd });
    }
  });
}
```

Keep the Magnet thin: **binding + transport selection only**. All logic stays in the execute function.

### 6. Register in harness.toml

Add to `harness/harness.toml`:
...
npm run check:structure      # Enforces module/source layout
...
**Trap 3:** Bundling multiple registered tools into one Magnet
...
3. **Place:** `harness/modules/tools/markdown-to-slides/pi/`
...
3. Place in `modules/<primitives>/<name>/pi/`
4. Write `.toml` descriptor
5. Wire Magnet (thin binding layer)
6. Register in `harness.toml` with `source = "pi"`
7. Gate with `npm run build/test/check/inspect`
