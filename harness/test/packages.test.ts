...
harness = "domain-harness/general/harness.toml"

[[profiles]]
name = "scrna"
description = "Single-cell RNA-seq resources."
extends = ["general"]
harness = "domain-harness/task/scrna/harness.toml"
`,
			);
			await writeText(
				join(packagesRoot, "AutOmicScience", "domain-harness", "general", "harness.toml"),
				`name = "omics-general"
...
`,
			...
			await writeText(
				join(packagesRoot, "AutOmicScience", "domain-harness", "task", "scrna", "harness.toml"),
				`name = "omics-scrna"
...
`,
			...
			await writeText(
				join(packagesRoot, "AutOmicScience", "domain-harness", "general", "mcp", "process", "omics-preflight.toml"),
				`kind = "mcp"
name = "omics_preflight"
`,
			);
			await writeText(
				join(packagesRoot, "AutOmicScience", "domain-harness", "general", "skills", "omics-shared", "SKILL.md"),
				`---
name: omics-shared
...
`,
			...
			expect(overlay.componentMap.get("skill:omics-shared")?.path).toBe(
				join(packagesRoot, "AutOmicScience", "domain-harness", "task", "scrna", "skills", "omics-shared-scrna"),
			);
...
`,
	...
	it("loads the migrated AutOmicScience package profiles", async () => {
		...
		expect(overlay.componentMap.get("skill:omics-shared")?.path).toBe(
			join(repoRoot, "packages", "AutOmicScience", "skills", "omics", "omics-shared"),
		);
		expect(overlay.componentMap.get("skill:omics-scrna")?.path).toBe(
			join(repoRoot, "packages", "AutOmicScience", "skills", "omics", "omics-scrna"),
		);
		expect(overlay.componentMap.get("skill:omics-multiome")?.path).toBe(
			join(repoRoot, "packages", "AutOmicScience", "skills", "omics", "omics-multiome"),
		);
		...
		expect(overlay.componentMap.get("tool:omics_runtime")?.path).toBe(
			join(repoRoot, "packages", "AutOmicScience", "domain-harness", "general", "tools", "omics-runtime.toml"),
		);
		expect(overlay.resources.skillPaths.map((resource) => resource.name).sort()).toEqual([
			"omics-multiome",
			"omics-scrna",
...
`,
			...
			await writeText(
				join(packagesRoot, "ProcessDomain", "domain-harness", "general", "harness.toml"),
...
`,
			...
			await writeText(
				join(packagesRoot, "ProcessDomain", "domain-harness", "general", "tools", "echo-process.toml"),
				`kind = "tool"
name = "echo_process"
...
`,
			...
			await writeText(
				join(packagesRoot, "ProcessDomain", "domain-harness", "general", "tools", "echo-process.mjs"),
...
`,
			...
			await writeText(
				join(packagesRoot, "EnvDomain", "domain-harness", "general", "harness.toml"),
				`name = "env-general"
...
`,
			...
			await writeText(
				join(packagesRoot, "EnvDomain", "domain-harness", "general", "tools", "env-probe.toml"),
				`kind = "tool"
name = "env_probe"
...
`,
			...
			await writeText(
				join(packagesRoot, "EnvDomain", "domain-harness", "general", "tools", "env-probe.mjs"),
...
`,
			...
			await writeText(
				join(packagesRoot, "ScriptDomain", "domain-harness", "general", "harness.toml"),
				`name = "script-general"
...
`,
			...
			await writeText(
				join(packagesRoot, "ScriptDomain", "domain-harness", "general", "tools", "node-script.toml"),
				`kind = "tool"
name = "node_script"
...
`,
			...
			await writeText(
				join(packagesRoot, "ScriptDomain", "domain-harness", "general", "tools", "node-script.js"),
				`let input = "";
...
`,
			...
			await writeText(
				join(packagesRoot, "PythonDomain", "domain-harness", "general", "harness.toml"),
				`name = "python-general"
...
`,
			...
			await writeText(
				join(packagesRoot, "PythonDomain", "domain-harness", "general", "tools", "python-echo.toml"),
				`kind = "tool"
name = "python_echo"
...
`,
			...
			await writeText(
				join(packagesRoot, "PixiDomain", "domain-harness", "general", "harness.toml"),
				`name = "pixi-general"
...
`,
			...
			await writeText(
				join(packagesRoot, "PixiDomain", "domain-harness", "general", "tools", "pixi-python.toml"),
				`kind = "tool"
name = "pixi_python"
...
`,
			...
			await writeText(
				join(packagesRoot, "NoEnvDomain", "domain-harness", "general", "harness.toml"),
				`name = "no-env-general"
...
`,
			...
			await writeText(
				join(packagesRoot, "NoEnvDomain", "domain-harness", "general", "tools", "no-env-python.toml"),
				`kind = "tool"
name = "no_env_python"
...
`,
	...
	it("loads migrated AutOmicScience omics skills with the harness skill loader", async () => {
		...
		const result = await loadSkills(env, "packages/AutOmicScience/skills/omics");
...
[[profiles]]
name = "task"
harness = "domain-harness/task/harness.toml"
`,
			);
			await writeText(
				join(packagesRoot, "StrictDomain", "domain-harness", "task", "harness.toml"),
				`name = "strict-task"
