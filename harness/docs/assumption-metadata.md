...
pruned. Core runtime (`core/loop`, `core/session`, `core/types`,
`core/messages`, `core/env`, `core/utils`) is architectural bedrock, not a
model-compensating assumption, and is **not** annotated. Contract modules
(`messages`, `types`) and pure assembly (`hcp-*`, `catalog`) are likewise out
of scope.
...
file header of `modules/tools-search/tool-search.ts`. If it is ever promoted to
...
| **Tool** (bash, edit, read, write, grep, find, ls, lsp, ssh, todo, web-search, web-fetch, show) | **No** | A tool is the model's hands, not a compensation. A smarter model still cannot read/write files or run commands without one, so a tool never becomes dead weight. Tools carry `[parameters]`/`[exports]`, not `[assumption]`. |
...
| **Resource** (skill, `system-prompt`, brand, theme, prompt content, `append-system-prompt`, a package's `SYSTEM.md`) | **No** | Content merged at assembly, no code builder — the four-primitive table lists `system-prompt` and `skill` as Resources. The capability that *loads* content may compensate for a limitation; the content module itself does not. |
| **Prompt template capability** (`prompt-template` module) | **Yes** | It is in `CAPABILITY_KINDS` (has a builder). Individual template files do not carry the block. |
...
| **Transport / infra** (`hcp-process/*.toml`, `catalog`, `hcp-*`, `overlay`, `registry`) | **No** | Assembly/transport wiring, not a model-compensating capability. |
| **Core runtime** (`core/loop`, `core/session`, `core/types`, `core/messages`, `core/env`, `core/utils`) | **No** | Architectural bedrock; exists regardless of model capability. |
| **Packaged component** (`packages/<Name>/…`) | **By primitive**, same rules | A packaged capability → yes; a packaged tool/skill/system-prompt/runtime/env → no. Domain tools provide access the model *structurally* lacks (external DBs, compute), which is not a model *weakness* that improves away. |

Executable source of truth: a component carries `[assumption]` **iff it is a
Capability primitive** — a `CapabilitySourceMagnet` with a `build()` fn. Today
that is `CAPABILITY_KINDS` (`hcp-client/overlay/package-overlay.ts`:
`compaction, context, hook, memory, policy, prompt-template, runtime, sandbox`)
plus `multiagent`. Note `multiagent` is wired through the trunk barrel
(`hcp-client/assembly/sources.ts`) and is deliberately *not* in the
package-overlay set: that set governs whether a **package** may contribute or
override a capability kind, which is a narrower question than "is this a
capability". Everything else — Tools, Resources (including `skill` and
`system-prompt`), sources, config, transport, and core — is out of scope by
design.

`scripts/audit-assumptions.mjs --check` enforces this rule mechanically
(`npm run check:assumptions`): it parses `CAPABILITY_KINDS` out of
`package-overlay.ts` at runtime rather than re-declaring it, so the two cannot
drift; `multiagent` is the one explicitly-listed extra. The parser it uses for
...
(`modules/<module>/<module>.toml`). All fields are optional and additive; the
