...
export interface McpToolsCacheKeyInput {
 ...
 */
async function binaryIdentity(command: string, args: string[]): Promise<{ fingerprint?: string }> {
	const parts: string[] = [];
 ...
 */
function cacheFileName(serverName: string, command: string, args: string[]): string {
...
export interface McpToolsCacheOptions {
	/** Directory where cache files live. Created on demand. */
	cacheDir: string;
	serverName: string;
	client: Pick<McpStdioClientOptions, "command" | "args" | "env">;
}
...
export async function readMcpToolsCache(options: McpToolsCacheOptions): Promise<McpToolSchema[] | undefined> {
	...
	const filePath = join(options.cacheDir, cacheFileName(options.serverName, options.client.command, args));
	...
	const identity = await binaryIdentity(options.client.command, args);
