...
**Maturity: REFERENCE** — hand-rolled distance + correlation in `run_python` (no snapATAC2 peak-gene function exists). The **scATAC-only** path (gene activity / co-accessibility); for paired multiome with real RNA, see `omics-multiome` (SCENIC+ region-gene links).
...
- **`observe_figure`** — a heatmap or network diagram of top peak-gene links (e.g., TF-gene pairs where the peak overlaps a TF motif). Are the links biologically plausible?
