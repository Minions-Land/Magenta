...
**Maturity: REFERENCE** — no compute subcommand; a few lines of `muon` in `run_python` (ground via `evidence_from_kernel_cell`). Produce **one cell representation** fusing RNA + ATAC for joint clustering and visualization. Works on the **MuData** directly.
